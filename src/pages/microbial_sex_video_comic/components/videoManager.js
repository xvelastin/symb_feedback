...
export const VIDEOS_TO_PLAY = 12;
...
export function VideoManager({}) {
    ...
    function ToggleVideoVisibility(videoElement, visible) {
        
        videoElement.style.opacity = visible ? 1: 0;
        setTimeout(() => {
                videoElement.style.pointerEvents = visible ? "auto" : "none";
            },
            visible ? 2000 : 0);
    }
    ...
    function PlayGalleryVideos() {
        ...
        videoElements.current.forEach((video) => {
            ...
            ToggleVideoVisibility(video, true);
        })
    }
    ...
    function StopGalleryVideos() {
        videoElements.current.forEach((video) => {
            ...
            ToggleVideoVisibility(video, false);
        })
    }
    ...
    function StopFullScreenVideo() {
        if (!fullScreenVideoRef.current) return;
        console.log("StopFullScreenVideo")
        ...
        <>
            ...
            <div id={"focusView"}
                 style={{
                     ...
                     opacity: displayFullScreenVideo ? 1 : 0,
                     pointerEvents: displayFullScreenVideo ? "auto" : "none",
                     transition: "2s opacity ease-in-out",

                 }}>
                <FullScreenVideo
                    src={focusedVideoSrc ? focusedVideoSrc : PATH_TO_VIDEOS + "/video_10.mp4"}
